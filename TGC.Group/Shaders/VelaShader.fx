...
float4 psDefault(VS_OUTPUT input) : COLOR0
{
	...
	float flickering = 0.035*frac(time*3);

	float xAlCuadrado = pow(position.x,2+flickering)/0.050;
	float yAlCuadrado = pow(position.y+0.85,2);

	float suma = (xAlCuadrado + yAlCuadrado)/0.05;
